...
class KRCNNConvDeconvUpsampleHead_custom(BaseKeypointRCNNHead, nn.Sequential):
    ...
    def __init__(self, input_shape, *, num_keypoints, conv_dims, **kwargs):
        ...
        for idx, layer_channels in enumerate(conv_dims, 1):
            if idx < 8: # the final
                ...
                offset_module = Conv2d(in_channels, 2 * 3 * 3, 3, stride=1, padding=1)
                self.add_module("conv_offset{}".format(idx), offset_module)
                dcn_module = DeformConv2d(in_channels, layer_channels, 3, stride=1, padding=1,bias=False,)
                self.add_module("conv_dcn{}".format(idx), dcn_module)
                self.add_module("conv_dcn_relu{}".format(idx), nn.ReLU())
    ...
    def layers(self, x):
        for idx,layer in enumerate(self):
            if isinstance(layer,DeformConv2d): #DeformConv2d):
                offset = nn.AdaptiveAvgPool2d((1))(x)
                offset = interpolate(offset, scale_factor=x.shape[-1] / offset.shape[-1],mode="bilinear", align_corners=False)
                x = layer(x_prev, offset)
                # try:
                #     x = layer(x_prev, offset)
                # except:
                #     x = x_prev # by pass some mistake
                #     print('wrong!')
                #     # pdb.set_trace()
            else:
                x_prev = x
